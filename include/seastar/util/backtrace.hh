...
// A single-frame backtrace.
// Used to avoid allocation of shared_backtrace to represent continuations.
using short_backtrace = frame;
...
// Each waiting task is represented either by the backtrace of its blocking point, if available, or by
// the address of its run_and_dispose() method.
class tasktrace {
public:
    using entry = std::variant<shared_backtrace, short_backtrace>;
